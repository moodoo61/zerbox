...
const localSettingsPath = path.join(__dirname, '..', '..', 'data', 'quran_local_settings.json');
const dataRootPath = path.join(__dirname, '..', '..', 'data');

function getSelectedReciterIds() {
    try {
        if (!fs.existsSync(localSettingsPath)) return [];
        const raw = fs.readJSONSync(localSettingsPath);
        const ids = raw && raw.selected_reciter_ids;
...
function hasLocalAudioFile(audioItem) {
    const link = audioItem && audioItem.link;
    if (typeof link !== 'string' || !link.startsWith('/data/')) return false;
    const rel = link.replace(/^\/data\//, '');
    const localPath = path.join(dataRootPath, rel);
...
export const getAudio = (req, res) => {
    ...
    try {
        ...
        const selectedReciters = getSelectedReciterIds();
        let filtered = audioData;

        if (selectedReciters.length > 0) {
            // اعرض المقرئين المحددين + أي مقرئ لديه ملف محلي موجود فعلياً
            filtered = audioData.filter((item) => {
                const id = item && item.id;
                return selectedReciters.includes(id) || hasLocalAudioFile(item);
            });
        } else {
            // إن لم يتم حفظ اختيار بعد: اعرض فقط من لديه ملفات صوتية محلية فعلية.
            filtered = audioData.filter((item) => hasLocalAudioFile(item));
        }
